...
def ka_lj_pair_hessian_geometry(
    positions: np.ndarray,
    *,
    particle_types: np.ndarray,
    box_lengths: np.ndarray,
    ...
    """Return each active unordered KA pair and its potential Hessian."""
    ...
    particle_i, particle_j = np.triu_indices(len(positions), k=1)
    wrapped = np.mod(positions, box_lengths)
    displacement = wrapped[particle_i] - wrapped[particle_j]
    displacement -= box_lengths * np.rint(displacement / box_lengths)
    distance = np.linalg.norm(displacement, axis=1)
    if np.any(distance <= 1e-10):
        raise ValueError("distinct particles must not overlap")
    sigma = _SIGMA[particle_types[particle_i], particle_types[particle_j]]
    active = distance < _CUTOFF_SCALE * sigma
    particle_i = particle_i[active]
    particle_j = particle_j[active]
    displacement = displacement[active]
    distance = distance[active]
    sigma = sigma[active]
    epsilon = _EPSILON[particle_types[particle_i], particle_types[particle_j]]
